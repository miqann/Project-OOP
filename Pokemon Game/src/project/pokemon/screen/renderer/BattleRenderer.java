package project.pokemon.screen.renderer;
...
import project.pokemon.Settings;
...
public class BattleRenderer {
	...
	public void render(SpriteBatch batch, BattleAnimation animation, BATTLE_PARTY primarilyAnimated) {
		...
		if (animation != null) {
			if (primarilyAnimated == BATTLE_PARTY.PLAYER) {
				...
				playerY = platformYOrigin + playerPokemon.getHeight() / 2 * Settings.SCALE
						- playerPokemon.getHeight() / 2 * Settings.SCALE * playerHeight;
				...
				opponentY = platformYOrigin + opponentPokemon.getHeight() / 2 * Settings.SCALE
						- opponentPokemon.getHeight() / 2 * Settings.SCALE * opponentHeight;
		...
		/* render platforms */
		batch.draw(platform, playerSquareMiddleX - platform.getRegionWidth() / 2 * Settings.SCALE, platformYOrigin,
				platform.getRegionWidth() * Settings.SCALE, platform.getRegionHeight() * Settings.SCALE);
		batch.draw(platform, opponentSquareMiddleX - platform.getRegionWidth() / 2 * Settings.SCALE, platformYOrigin,
				platform.getRegionWidth() * Settings.SCALE, platform.getRegionHeight() * Settings.SCALE);
		...
		if (playerPokemon != null) {
			...
			if (animation != null && animation.getPrimaryMask() != null && !animation.isFinished()) {
				this.renderWithMask(batch, pokemonTexture, animation.getPrimaryMask(), animation.getPrimaryMaskAmount(),
						playerX, playerY, playerWidth * playerPokemon.getWidth() * Settings.SCALE,
						playerHeight * playerPokemon.getHeight() * Settings.SCALE, true);
			} else {
				batch.draw(playerPokemon, playerX, playerY, playerWidth * playerPokemon.getWidth() * Settings.SCALE,
						playerHeight * playerPokemon.getHeight() * Settings.SCALE, 0, 0, playerPokemon.getWidth(),
						playerPokemon.getHeight(), true, false);
			}
		}
		if (opponentPokemon != null) {
			batch.setColor(1f, 1f, 1f, opponentAlpha);
			batch.draw(opponentPokemon, opponentX, opponentY,
					opponentWidth * opponentPokemon.getWidth() * Settings.SCALE,
					opponentHeight * opponentPokemon.getHeight() * Settings.SCALE, 0, 0, opponentPokemon.getWidth(),
					opponentPokemon.getHeight(), false, false);
		}
		...
		if (animation != null && !animation.isFinished()) {
			for (BattleSprite sprite : animation.getSprites()) {
				...
				float spriteY = 0f;
				if (primarilyAnimated == BATTLE_PARTY.PLAYER) {
					spriteX = playerSquareMiddleX + sprite.getX() * squareSize
							- sprite.getWidth() * sprite.getRegion().getRegionWidth() / 2;
					spriteY = playerSquareMiddleY + sprite.getY() * squareSize
							- sprite.getHeight() * sprite.getRegion().getRegionHeight() / 2;
				} else if (primarilyAnimated == BATTLE_PARTY.OPPONENT) {
					spriteX = opponentSquareMiddleX - sprite.getX() * squareSize
							- sprite.getWidth() * sprite.getRegion().getRegionWidth() / 2;
					spriteY = opponentSquareMiddleY + sprite.getY() * squareSize
							- sprite.getHeight() * sprite.getRegion().getRegionHeight() / 2;
				}
				batch.draw(sprite.getRegion(),
						...
						Settings.SCALE * sprite.getHeight() * sprite.getRegion().getRegionHeight(), 1f, 1f,
						sprite.getRotation());
				batch.setColor(1f, 1f, 1f, 1f);
			}
		}
	...
	private void renderWithMask(SpriteBatch batch, Texture texture, Texture mask, float maskAmount, float x, float y,
		...
		batch.draw(texture, x, y, width, height, 0, 0, texture.getWidth(), texture.getHeight(), flipX, false);
