...
import project.pokemon.model.YSortable;
	...
	public void render(SpriteBatch batch, Camera camera) {
		...
		for (int x = 0; x < world.getMap().getWidth(); x++) {
			for (int y = 0; y < world.getMap().getHeight(); y++) {
				...
				if (render != null) {
					batch.draw(render, (int) (worldStartX + x * Settings.SCALED_TILE_SIZE),
							(int) (worldStartY + y * Settings.SCALED_TILE_SIZE), (int) (Settings.SCALED_TILE_SIZE),
		...
		for (YSortable loc : forRendering) {
			...
			if (loc instanceof Actor) {
				...
				batch.draw(sprite,
						...
						Settings.SCALED_TILE_SIZE * widthInTiles, Settings.SCALED_TILE_SIZE * heightInTiles);
				continue;
			} else {
