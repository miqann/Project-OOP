...
import project.battle.animation.FaintingAnimation;
import project.battle.animation.PokeballAnimation;
import project.battle.event.AnimationBattleEvent;
import project.battle.event.BattleEvent;
import project.battle.event.BattleEventPlayer;
import project.battle.event.BattleEventQueuer;
import project.battle.event.HPAnimationEvent;
import project.battle.event.NameChangeEvent;
import project.battle.event.PokeSpriteEvent;
...
public class Battle implements BattleEventQueuer {
	...
	public enum STATE {
		READY_TO_PROGRESS, SELECT_NEW_POKEMON, RAN, WIN, LOSE,;
	}
	...
	/**
	 * Sends out a new Pokemon, in the case that the old one fainted. This will NOT
	 * take up a turn.
	 * 
	...
	public void chooseNewPokemon(Pokemon pokemon) {
		this.player = pokemon;
		queueEvent(new HPAnimationEvent(BATTLE_PARTY.PLAYER, pokemon.getCurrentHitpoints(),
				pokemon.getCurrentHitpoints(), pokemon.getStat(STAT.HITPOINTS), 0f));
