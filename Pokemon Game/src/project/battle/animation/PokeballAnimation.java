package project.battle.animation;
...
public class PokeballAnimation extends BattleAnimation {
	...
	public void initialize(AssetManager assetManager, TweenManager tweenManager) {
		 ...
		 */
		Tween.to(pokeballSprite, BattleSpriteAccessor.X, pokeballOpen).target(0f).ease(Linear.INOUT)
				.start(tweenManager);
		Tween.to(pokeballSprite, BattleSpriteAccessor.Y, 0.25f).target(1f).ease(Sine.OUT).start(tweenManager);
		Tween.to(pokeballSprite, BattleSpriteAccessor.Y, 0.25f).target(0f).ease(Sine.IN).delay(0.25f)
				.start(tweenManager);

		/*
		 * ROTATION OF POKEBALL
		 */
		Tween.to(pokeballSprite, BattleSpriteAccessor.ROTATION, 0.5f).target(2f * 360f).ease(Linear.INOUT)
				.start(tweenManager);

		Tween.to(pokeballSprite, BattleSpriteAccessor.ALPHA, 0.25f).target(0f).ease(Linear.INOUT).delay(0.7f)
				.start(tweenManager);
		...
		Tween.to(this, BattleAnimationAccessor.PRIMARY_WIDTH, 0.5f).target(1f).delay(pokeballOpen).start(tweenManager);
		Tween.to(this, BattleAnimationAccessor.PRIMARY_HEIGHT, 0.5f).target(1f).delay(pokeballOpen).start(tweenManager);
		...
		Tween.to(this, BattleAnimationAccessor.PRIMARY_MASK_AMOUNT, 1f).target(0f).ease(Linear.INOUT)
				.delay(pokeballOpen + 0.3f).start(tweenManager);
	}

	private void addEffectSprite(Animation anim, float endX, float endY, TweenManager tweenManager) {
		AnimatedBattleSprite effectSprite = new AnimatedBattleSprite(anim, 0f, 0f, 1f, 1f, pokeballOpen);
		effectSprite.setAnimationMode(PlayMode.LOOP);
		effectSprite.setAlpha(0f);
		addSprite(effectSprite);

		Tween.to(effectSprite, BattleSpriteAccessor.ALPHA, 0f).target(1f).delay(pokeballOpen).start(tweenManager);

		Tween.to(effectSprite, BattleSpriteAccessor.Y, 1f).target(endY).ease(Linear.INOUT).delay(pokeballOpen)
				.start(tweenManager);

		Tween.to(effectSprite, BattleSpriteAccessor.X, 1f).target(endX).ease(Linear.INOUT).delay(pokeballOpen)
				.start(tweenManager);
	}
