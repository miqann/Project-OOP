...
import project.pokemon.util.TextureAnimation;
import project.pokemon.util.TextureAnimation.PlayMode;
...
public class StatChangeAnimation extends BattleAnimation {
	...
	public void initialize(AssetManager assetManager, TweenManager tweenManager) {
		...
		Tween.to(this, BattleAnimationAccessor.PRIMARY_MASK_AMOUNT, 0.25f).target(0.8f).ease(Linear.INOUT)
				.start(tweenManager);
		Tween.to(this, BattleAnimationAccessor.PRIMARY_MASK_AMOUNT, 0.25f).target(0f).delay(0.75f).ease(Linear.INOUT)
				.start(tweenManager);
	}
